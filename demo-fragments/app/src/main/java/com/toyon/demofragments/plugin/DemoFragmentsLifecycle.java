...
import com.toyon.demofragments.DemoFragmentsMapComponent;
...
public class DemoFragmentsLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = "DemoFragmentsLifecycle";

    public DemoFragmentsLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        DemoFragmentsLifecycle.this.overlays
                .add(new DemoFragmentsMapComponent());

        // create components
        Iterator<MapComponent> iter = DemoFragmentsLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(DemoFragmentsLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        DemoFragmentsLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,
