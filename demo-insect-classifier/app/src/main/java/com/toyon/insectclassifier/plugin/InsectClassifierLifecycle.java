...
import com.toyon.insectclassifier.ClassifyInsectMapComponent;
...
public class InsectClassifierLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = InsectClassifierLifecycle.class.getSimpleName();

    public InsectClassifierLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        InsectClassifierLifecycle.this.overlays
                .add(new ClassifyInsectMapComponent());

        // create components
        Iterator<MapComponent> iter = InsectClassifierLifecycle.this.overlays
                .iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(InsectClassifierLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        InsectClassifierLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,
