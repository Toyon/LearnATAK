...
import com.toyon.plantclassifier.PlantClassifierMapComponent;
...
/**
 * Main plugin entry point of Plant Classifier Plugin handling ATAK's lifecycle callbacks and
 * loads main MapComponents
 */
public class PlantClassifierLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = PlantClassifierLifecycle.class.getSimpleName();

    public PlantClassifierLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        PlantClassifierLifecycle.this.overlays.add(new PlantClassifierMapComponent());
        // create components
        Iterator<MapComponent> iter = PlantClassifierLifecycle.this.overlays.iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(PlantClassifierLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        PlantClassifierLifecycle.this.mapView);
            } catch (Exception e) {
                Log.w(TAG,
