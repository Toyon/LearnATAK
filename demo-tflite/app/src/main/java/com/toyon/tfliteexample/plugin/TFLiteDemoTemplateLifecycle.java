...
public class TFLiteDemoTemplateLifecycle implements Lifecycle {

    private final Context pluginContext;
    private final Collection<MapComponent> overlays;
    private MapView mapView;

    private final static String TAG = TFLiteDemoTemplateLifecycle.class.getSimpleName();

    public TFLiteDemoTemplateLifecycle(Context ctx) {
    ...
    public void onCreate(final Activity arg0,
        ...
        TFLiteDemoTemplateLifecycle.this.overlays
                .add(new TFLiteDemoMapComponent());

        // create components
        Iterator<MapComponent> iter = TFLiteDemoTemplateLifecycle.this.overlays.iterator();
        MapComponent c;
        while (iter.hasNext()) {
            c = iter.next();
            try {
                c.onCreate(TFLiteDemoTemplateLifecycle.this.pluginContext,
                        arg0.getIntent(),
                        TFLiteDemoTemplateLifecycle.this.mapView);
            } catch (Exception e) {
